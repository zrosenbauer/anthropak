// Config loading and validation for CLI (duplicated from hook package)
// CLI needs its own copy since it runs in different environment
...
export async function loadConfig(rootDir: string): Promise<ConfigLoadResult> {
  // eslint-disable-next-line no-await-in-loop -- Sequential file loading required
...
export function validateConfig(raw: unknown): ConfigLoadResult {
  ...
  // Validate cli_tools section if present
  if (hasCliTools) {
...
function validateEcosystemSection(
  ...
  // For cli_tools section, validate entries deeply
  if (name === "cli_tools") {
    const validatedRequired = [];
    const validatedOptional = [];
