...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/cli/src/templates/dependencies-yaml.ts"
      provides: "TypeScript module exporting template string for new nested schema"
      contains: "version: 1"
...
Add `ts-pattern` (^5.5.0), `es-toolkit` (^1.44.0), and `confbox` (^0.1.8) to dependencies. `confbox` must be a direct dependency of CLI (not relying on workspace hoisting from hook) because CLI needs it for its own config-loader.ts to parse YAML/JSON/JSONC config files. Remove `liquidjs` from dependencies (we will replace the Liquid template with a simple string template — the template is trivially simple YAML and doesn't need a full template engine; this reduces dependencies and aligns with the es-toolkit philosophy of not over-engineering). Run `pnpm install`.
...
**Create packages/cli/src/templates/dependencies-yaml.ts (replaces old .liquid template — delete dependencies.yaml.liquid if it exists):**
This is a TypeScript module that exports the template as a plain string constant. No template engine needed since the template has no dynamic parts.

The file should export a `DEPENDENCIES_YAML_TEMPLATE` constant containing the new nested schema YAML per CONTEXT.md. The YAML content must include:
- `version: 1` at top level
- `plugins:` section with `required: []` and `optional: []`
- Commented examples showing all available fields (plugin, github, description, install)
- No `marketplace` field (dropped per CONTEXT.md)
- No `cli` or `mcp` sections in template (they'll be added in Phase 2/3)
- Header comments with project name and docs link
...
- Remove liquidjs dependency entirely — no template engine needed
- `renderDependenciesYaml(): string` — import `DEPENDENCIES_YAML_TEMPLATE` from `../templates/dependencies-yaml.js` and return it directly (no Liquid rendering needed since template has no dynamic parts). Alternatively can return it from `.generated/index.js` if build-assets embeds it — either approach works, prefer the direct import from the `.ts` source for clarity.
...
- The script itself can use sync fs (it's a build script, not runtime code)
  ...
  <verify>
Run `pnpm typecheck` from packages/cli — should pass (may need .generated/ files to exist first; run `pnpm build:assets` after hook is built). Verify no try-catch: `grep -rn "try {" packages/cli/src/lib/` returns nothing. Verify liquidjs removed from package.json: `grep -c "liquidjs" packages/cli/package.json` returns 0. Verify template has `version: 1`: `grep "version: 1" packages/cli/src/templates/dependencies-yaml.ts`. Verify old .liquid file deleted: `test ! -f packages/cli/src/templates/dependencies.yaml.liquid`.
  </verify>
  <done>
...
1. Load config using the same loading logic as the hook (import from shared config loader or duplicate the loading/validation logic locally — since CLI can't import from hook at runtime, duplicate the config loading code or create a shared loader in cli/src/lib/):
   ...
   Actually, simpler approach: the CLI can import confbox directly. **Add `confbox` as a direct dependency in packages/cli/package.json** (do NOT rely on workspace hoisting from the hook package — CLI must declare its own dependency for correctness and to survive isolated installs). Then duplicate the loadConfig + validateConfig functions in a cli/src/lib/config-loader.ts file, using the same types from cli/src/types.ts.
